...
import { LayoutDashboard, ListOrdered, Bot, PiggyBank, RefreshCw } from "lucide-react";
  ...
  return (
    <aside className="glass-sidebar p-4 md:p-6 md:static md:w-auto fixed left-0 top-0 h-full w-72 z-50 overflow-y-auto">
      ...
      <nav className="space-y-2">
        ...
        <NavLink to="/budgets" className={({ isActive }) => `flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-300 ${isActive ? "bg-black/40 border border-amber-500/30" : "hover:bg-black/30"}` }>
          <PiggyBank size={16} className="text-amber-400" />
          <span>Orçamentos</span>
        </NavLink>
        <NavLink to="/recurrings" className={({ isActive }) => `flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-300 ${isActive ? "bg-black/40 border border-amber-500/30" : "hover:bg-black/30"}` }>
